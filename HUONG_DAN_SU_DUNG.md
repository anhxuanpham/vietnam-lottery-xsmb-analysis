...
gold/releases/run-id=<run>/ Parquet Gold bất biến theo phiên bản
exports/csv/run-id=<run>/ CSV bất biến do job tuần tạo cho BI
control/     snapshot trạng thái ngày + pointer compact
quality/     Báo cáo kiểm tra chất lượng
manifests/   Trạng thái run và manifest phiên bản mới nhất
```

Không dùng Bronze trực tiếp cho dashboard. Luôn đọc `manifests/latest.json`, kiểm tra size/SHA-256 rồi mở đúng Gold
object nó tham chiếu. BI dùng `exports/csv/latest.json`; không hard-code `gold/latest/`.
...
1. Dùng `exports/csv/latest.json` làm nguồn CSV cho BI; mỗi tuần job tạo một snapshot mới.
2. Kiểm tra `manifests/latest.json` và checksum trước khi refresh dashboard.
...
Các workflow production định kỳ và repair thủ công đã có sẵn:
...
Tạo đủ bảy secret bucket/credential bắt buộc:
...
| `R2_BACKUP_BUCKET_NAME` | Bucket backup release, ví dụ `vietnam-lottery-backup` |
...
Khuyến nghị tạo failure domain riêng cho backup bằng `R2_BACKUP_ACCOUNT_ID`, `R2_BACKUP_ACCESS_KEY_ID`,
`R2_BACKUP_SECRET_ACCESS_KEY` và tùy chọn `R2_BACKUP_ENDPOINT_URL`. Nếu để trống toàn bộ, backup dùng credential chính.

Backup này chỉ khôi phục boundary phục vụ Gold, manifest và ControlState. Nó không chứa Bronze/Silver, không được
restore đè primary R2, và lake restore có marker chặn ETL để tránh publish lịch sử bị cắt ngắn.
...
- [ ] Bảy GitHub Secrets bucket/credential bắt buộc đã được tạo.
